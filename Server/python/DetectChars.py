...
###################################################################################################
def loadKNNDataAndTrainKNN(): # knn 알고리즘
...
###################################################################################################
def detectCharsInPlates(listOfPossiblePlates): # 글자영역에서 문자 검출
    intPlateCounter = 0
    imgContours = None
    contours = []
...
###################################################################################################
def findPossibleCharsInPlate(imgGrayscale, imgThresh): # 문자 가능 후보 추출 및 저장
...
###################################################################################################
def checkIfPossibleChar(possibleChar): # 가능 문자 check
...
###################################################################################################
def findListOfListsOfMatchingChars(listOfPossibleChars): # 매칭 문자열 가운데서 가장 긴 것을 선택
...
###################################################################################################
def findListOfMatchingChars(possibleChar, listOfChars): # 매칭 문자의 각 그룹은 글자연영으로 인식됨.
...
###################################################################################################
# use Pythagorean theorem to calculate distance between two chars # 피타고라스 이론 적용 위해
...
def angleBetweenChars(firstChar, secondChar): # 삼각법(삼각측량법)
...
def removeInnerOverlappingChars(listOfMatchingChars): # 매칭 문자들을 정렬(왼->오) 및 중첩된 문자 제거
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars): # 영역 안의 문자 인식
